...
def refresh_creds(ctx):
    ...
    res = requests.post(url='https://api.outreach.io/oauth/token', data=data)

    creds = {'access_token': res.json()['access_token'], 'refresh_token': res.json()['refresh_token']}

    ctx.update_creds(creds)
